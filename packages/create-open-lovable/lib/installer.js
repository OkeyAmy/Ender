...
async function createEnvFile(projectPath, sandbox, answers) {
  ...
  if (answers.scrapegraphApiKey) {
    envContent += `SCRAPEGRAPH_API_KEY=${answers.scrapegraphApiKey}\n`;
  } else {
    envContent += `# SCRAPEGRAPH_API_KEY=your_scrapegraph_api_key_here  # Optional alternative scraper\n`;
  }
  envContent += '\n';
...
async function createEnvExample(projectPath, sandbox) {
  ...
  envContent += `# SCRAPEGRAPH_API_KEY=your_scrapegraph_api_key_here  # Optional alternative at https://scrapegraphai.com\n\n`;
