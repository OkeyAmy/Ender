...
import ScraperSelector from "@/components/shared/ScraperSelector";
...
export default function HomePage() {
  ...
  const [selectedScraper, setSelectedScraper] = useState<string>(appConfig.scraper.defaultScraper);
  ...
  const chainOptions = [
    { id: "solana" as const, name: "Solana", blurb: "Ultra-fast, low-fee mainnet" },
    { id: "celo" as const, name: "Celo", blurb: "Mobile-first, carbon negative" },
  ];
  ...
  useEffect(() => {
    ...
    if (creationMode !== "url" && enable3DDepth) {
      setEnable3DDepth(false);
    }
  }, [creationMode, enable3DDepth]);
  ...
  const handleSubmit = async (selectedResult?: SearchResult) => {
    if (creationMode === "text") {
      ...
      try {
        ...
        sessionStorage.setItem('enable3DDepth', enable3DDepth ? 'true' : 'false');
    ...
    if (!inputValue) {
      toast.error("Enter a URL or search phrase to replicate.");
      return;
    }

    sessionStorage.setItem('creationMode', 'url');
    sessionStorage.setItem('selectedChain', selectedChain);
    sessionStorage.setItem('enable3DDepth', enable3DDepth ? 'true' : 'false');
  ...
  return (
    <HeaderProvider>
      <div className="min-h-screen bg-background-base">
        ...
        <section className="overflow-x-clip" id="home-hero">
          <div className="pt-28 lg:pt-254 lg:-mt-100 pb-115 relative" id="hero-content">
            ...
            <div className="relative container px-16">
              <HomeHeroBadge />
              <HomeHeroTitle />
              <p className="text-center text-body-large max-w-3xl mx-auto">
                Vibe-code blockchain dapps in minutes
              </p>
            </div>
          </div>
              ...
              <div className="rounded-20 -mt-30 lg:-mt-30">
                ...
                >
                  <div className="px-16 pt-16">
                    <div className="inline-flex gap-3 bg-black-alpha-4 rounded-12 p-4">
                      <button
                        type="button"
                        onClick={() => setCreationMode("text")}
                        className={`button relative rounded-10 px-14 py-8 text-label-medium font-medium transition-all ${
                          creationMode === "text"
                            ? "button-primary text-accent-white active:scale-[0.998]"
                            : "bg-transparent text-black-alpha-48 hover:text-accent-black"
                        }`}
                      >
                        {creationMode === "text" && (
                          <div className="button-background absolute inset-0 rounded-10 pointer-events-none" />
                        )}
                        <span className="relative">Create from Text</span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span className="relative">Replicate from URL</span>
                      </button>
                    </div>
                  </div>
                  ...
                  {creationMode === "text" ? (
                    <div className="px-16 pb-16 pt-16">
                      ...
                      <div className="bg-white rounded-20 border border-black-alpha-8">
                        {/* Textarea area */}
                        <div className="p-16 pb-12">
                          <textarea
                            className="w-full resize-none bg-transparent text-accent-black placeholder:text-black-alpha-48 focus:outline-none focus:ring-0 text-body-input"
                            style={{ minHeight: 100 }}
                            placeholder="Build your idea onchain by chatting with AI"
                            value={textBrief}
                            onChange={(e) => setTextBrief(e.target.value)}
                          />
                        </div>

                        {/* Bottom section with dropdowns and send button */}
                        <div className="flex items-center gap-8 p-16 pt-12 border-t border-black-alpha-8">
                          {/* Left side: Blockchain Icon + Dropdown */}
                          <div className="flex items-center gap-6">
                            <svg 
                              ...
                              className="opacity-60 text-black-alpha-48 flex-shrink-0"
                            >
                            ...
                            <div className="relative">
                              <select
                                value={selectedChain}
                                onChange={(e) => setSelectedChain(e.target.value as "solana" | "celo")}
                                className="bg-gray-100 text-gray-800 text-body-small px-10 py-6 pr-24 rounded-8 border border-gray-200 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 appearance-none cursor-pointer"
                              >
                          ...
                          {/* Middle: AI Model Dropdown */}
                          <div className="flex-1 relative">
                            <select
                              value={selectedModel}
                              onChange={(e) => setSelectedModel(e.target.value)}
                              className="w-full bg-gray-100 text-gray-800 text-body-small px-10 py-6 pr-24 rounded-8 border border-gray-200 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 appearance-none cursor-pointer"
                            >
                              {models.map((model) => (
                          ...
                          <button
                            ...
                            className={`w-32 h-32 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${
                              textBrief.trim() && !isTextSubmitting
                          ...
                          >
                            {isTextSubmitting ? (
                              ...
                              <svg 
                                ...
                                fill="none" 
                                xmlns="http://www.w3.org/2000/svg"
                                className={textBrief.trim() ? "text-white" : "text-gray-400"}
                              >
                                <path d="M14 2L6 10M14 2L10 14L6 10M14 2L2 6L6 10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                              </svg>
                            )}
                          </button>
                        </div>
                    ...
                    <div className="px-16 pb-16">
                          ...
                          <>
                            ...
                            >
                              <HeroInputSubmitButton 
                                dirty={url.length > 0} 
                                buttonText={isURL(url) ? 'Clone Site' : 'Search'} 
                                disabled={isSearching}
                              />
                            </div>
                          </>
                      ...
                      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
                        isValidUrl ? 'max-h-[220px] opacity-100' : 'max-h-0 opacity-0'
                      }`}>
                        <div className="p-[28px]">
                          <div className="border-t border-gray-100 bg-white">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                {styles.map((style, index) => (
                                  <button
                                    ...
                                    className={`
                                      py-3 px-3 rounded-8 text-[11px] font-medium border transition-all text-center
                            ...
                            <div className={`mb-2 transition-all duration-300 transform ${
                              ...
                              <button
                                type="button"
                                onClick={() => setEnable3DDepth((prev) => !prev)}
                                className={`
                                  flex items-center justify-between w-full rounded-12 border px-12 py-10 transition-all
                                  ${
                                    enable3DDepth
                                      ? 'border-orange-500 bg-orange-50 text-orange-900'
                                      : 'border-gray-200 hover:border-gray-300 bg-white text-gray-700'
                              ...
                              >
                                <div className="flex items-center gap-8">
                                  ...
                                  <div>
                                    <div className="text-label-small font-semibold">3D depth layers</div>
                                    <div className="text-[10px] text-black-alpha-48">Add parallax-ready shadows</div>
                                  </div>
                                </div>
                                <span className={`text-[10px] font-semibold ${enable3DDepth ? 'text-orange-600' : 'text-black-alpha-48'}`}>
                                  {enable3DDepth ? 'Enabled' : 'Optional'}
                                </span>
                              </button>
                            </div>
                            ...
                            <div className={`flex gap-3 mt-2 pb-4 transition-all duration-300 transform ${
                              ...
                              <select
                                value={selectedModel}
                                onChange={(e) => setSelectedModel(e.target.value)}
                                className="px-12 py-10 text-[11px] font-medium text-gray-700 bg-white rounded-8 border border-gray-200 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
                              >
                                {models.map((model) => (
                                  <option key={model.id} value={model.id}>
                                    {model.name}
                              ...
                              <input
                                type="text"
                                className="flex-1 px-12 py-10 text-[11px] text-gray-700 bg-gray-50 rounded-8 border border-gray-200 focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 placeholder:text-gray-400"
        ...
        {creationMode === "url" && showSearchTiles && hasSearched && (
